...
**取消任务创建**: 在创建任务过程中发送 \`取消\`
  ...
  if (COMMANDS.COMPLETE.includes(command) && !creating) {
    ...
    db.run(`UPDATE tasks SET completed = 1 WHERE id = ? AND user = ?`, [taskId, message.author.id], function (err) {
      if (err) return message.channel.send('❌ 标记任务失败');
    });
    return message.channel.send(`✅ 已标记任务 #${taskId} 为完成`);
  }
  ...
  if (COMMANDS.DELETE.includes(command) && !creating) {
    const taskId = parseInt(content.split(' ')[1]);
    if (isNaN(taskId)) {
      return message.channel.send(`❌ 删除格式：删除 <id>`);
    }
    db.run(`DELETE FROM tasks WHERE id = ? AND user = ?`, [taskId, message.author.id], function (err) {
      if (err) return message.channel.send('❌ 删除任务失败');
    });
    return message.channel.send(`🗑️ 已删除任务 #${taskId}`);
  }
});
